...
class PacketInLoad(base_tests.SimpleDataPlane):
    ...
    def runTest(self):
        ...
        for of_port in of_ports:
            for pkt, pt in [
               (simple_tcp_packet(pktlen=140), "simple TCP packet"),
               (simple_tcp_packet(), "simple tagged TCP packet"),
               (simple_eth_packet(), "simple Ethernet packet"),
               (simple_eth_packet(pktlen=140), "tiny Ethernet packet")]:
        ...
        while True:
            (response, raw) = self.controller.poll(ofp.POFT_PACKET_IN)
            if not response:
                break
            in_count += 1
