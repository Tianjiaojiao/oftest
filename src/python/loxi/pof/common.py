...
class table_resource_desc(loxi.OFObject):

    def __init__(self, device_id=None, type=None, tbl_num=None, key_len=None, total_size=None):
        if device_id != None:
    ...
    def pack(self):
        packed = []
        packed.append(struct.pack("!L", self.device_id)) # placeholder for len at index 0
        packed.append(struct.pack("!B", self.type))
        packed.append(struct.pack("!B", self.tbl_num))
        packed.append(struct.pack("!H", self.key_len))
        packed.append(struct.pack("!L", self.total_size))
        packed.append('\x00' * 4)
        return ''.join(packed)

    @staticmethod
    def unpack(reader):
        obj = table_resource_desc()
        obj.device_id = reader.read("!L")[0]
        obj.type = reader.read("!B")[0]
        obj.tbl_num = reader.read("!B")[0]
        obj.key_len = reader.read("!H")[0]
        obj.total_size = reader.read("!L")[0]
        reader.skip(4)
        return obj
    ...
    def pretty_print(self, q):
		...
		q.text(","); q.breakable()
		q.text("tbl_num = ");
		q.text("%#x" % self.tbl_num)
                q.text(","); q.breakable()
                q.text("key_len = ");
                q.text("%#x" % self.key_len)
                q.text(","); q.breakable()
                q.text("total_size = ");
                q.pp("%#x" % self.total_size)
            q.breakable()
        q.text('}')
"""
...
class group_stats_entry(loxi.OFObject):
    ...
    def __init__(self, group_id=None, ref_count=None, packet_count=None, byte_count=None, duration_sec=None, duration_nsec=None, bucket_stats=None):
        ...
        if bucket_stats != None:
            self.bucket_stats = bucket_stats#bucket_counter
        else:
            self.bucket_stats = []
        return
...
#-----common head for all hello elements, shown at "openflow-switch-v1.4.1.pdf", page 147, pofswitch do not support-----
...
class pof_match(loxi.OFObject):
    ...
    def unpack(reader):
        obj = pof_match()
        obj.field_id = reader.read("!H")[0]
        obj.offset = reader.read("!H")[0]
        _length = reader.read("!H")[0]
        orig_reader = reader
        reader = orig_reader.slice(_length, 4)
        return obj
    ...
    def pretty_print(self, q):
        ...
        with q.group():
            with q.indent(2):
                q.breakable()
                q.text("field_id = ");
                q.text("%#x" % self.field_id)
                q.text(",");
                q.breakable()
                q.text("offset = ");
                q.text("%#x" % self.offset)
            q.breakable()
        q.text('}')
...
#-----Configure/Modify behavior of a flow table, shown at "openflow-switch-v1.4.1.pdf", page 85-----
